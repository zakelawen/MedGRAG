# process_medmcqa_local_embedding.py
...
#             # 针对每条路径获取 embedding，并计算相似度（使用本地模型并并行）
#             path_embeddings = embedding_client.get_local_embeddings_concurrent(paths)
#             path_sim_list = []
#             for p, emb in zip(paths, path_embeddings):
#                 sim_score = cosine_similarity(context_embedding, emb)
#                 path_sim_list.append((p, sim_score))

#             # 按相似度降序
#             path_sim_list.sort(key=lambda x: x[1], reverse=True)
#             best_paths = path_sim_list[:10]
...
#                         context_embedding = embedding_client.get_local_embedding(context_text) if context_text else None
#                         node_paths_embeddings = embedding_client.get_local_embeddings_concurrent(node_paths)
#                         path_sim_list = []
#                         for p, emb in zip(node_paths, node_paths_embeddings):
#                             sim_score = cosine_similarity(context_embedding, emb)
#                             path_sim_list.append((p, sim_score))

#                         path_sim_list.sort(key=lambda x: x[1], reverse=True)
#                         best_paths = path_sim_list[:10]
#                         node_paths = [bp[0] for bp in best_paths]
...
def main():
    ...
    # 你的 API_KEY / 路径
    openai_api_key = "sk-kydgPim7qKMOHeX77f8fBd194c364a429106Ff96323b39E2"  # 请替换为实际的OpenAI API密钥
    ...
    input_jsonl = "/home/user1/zjs/实验/最终测试数据进行RAG实验/medmcqa.jsonl"
    ...
    try:
        ...
        openai_client = OpenAIClient(
            ...
            base_url="https://hk.xty.app/v1"  # 请根据实际情况调整
        )
