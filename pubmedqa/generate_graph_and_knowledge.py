...
# 加载环境变量
load_dotenv()
OPENAI_API_KEY = "sk-kydgPim7qKMOHeX77f8fBd194c364a429106Ff96323b39E2"  # 请确保安全存储
...
class OpenAIClientWrapper:
    ...
    def generate_medical_knowledge(self, question: str) -> str:
        """
        使用LLM生成相关的医学知识。
        """
        prompt = f"""
Task:
Please provide relevant medical knowledge that helps to understand or solve the problem based on the following medical question.Only output knowledge, the content should be useful, concise and correct.
...
"""
    ...
    def generate_graph_triples(self, question: str, knowledge: str) -> str:
        """
        使用LLM根据问题和相关知识生成图谱（三元组）。
...
def process_medqa_line(
    ...
    """
    logging.info(f"开始处理行 {line_number}：问题='{question}'")
    ...
    combined_summarized_definitions = question_summarized_definitions
    combined_filtered_paths = question_filtered_paths

    # 生成并保存GPT提供的相关医学知识
    medical_knowledge = llm_client.generate_medical_knowledge(question)
    ...
    # 生成知识图谱文本
    graph_triples_text = llm_client.generate_graph_triples(question, all_combined_knowledge)
...
def main():
    ...
    """
    # 配置路径
    input_jsonl = "/home/user1/zjs/实验/最终测试数据进行RAG实验/pubmedqa.jsonl"  # 请替换为实际路径
